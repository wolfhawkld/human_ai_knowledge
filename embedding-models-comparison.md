...
**Matryoshka Embedding** 是一种训练技术，让模型在一次推理后产出可任意截断的向量——从 1024 维截到 256 维甚至 64 维，无需重新推理或重新训练，召回率仅轻微下降。

**工作原理：** 训练时在多个维度级别（如 1024、512、256、128、64）同时施加对比学习损失，迫使模型学会"重要信息优先编码到前几维"。推理后，取 `vec[:d]` 即可获得任意维度的子向量。

**支持 Matryoshka 的主流模型：**

| 模型 | 原生维度 | 可选维度 |
|------|:-------:|---------|
| OpenAI text-embedding-3-large | 3072 | 256, 512, 768, 1024, 1536 |
| OpenAI text-embedding-3-small | 1536 | 256, 512, 768, 1024 |
| Nomic embed text v1.5 | 768 | 64, 128, 256, 512 |
| Jina embeddings v3 | 1024 | 32, 64, 128, 256, 512, 768 |
| Cohere embed-v4.0 | 1536 | 256, 512, 1024 |
| BGE-M3 | 1024 | 通过独立推理实现多维度 |
| stella_en_1.5B_v5 | 1024 | 支持多维度 |

**实践意义：**
- **向量库成本灵活**：高维向量存储成本与维度成正比，Matryoshka 可在不重新编码的情况下动态降维节省 50-90% 存储
- **精度 vs 速度取舍**：256 维通常可达 95%+ 的 1024 维召回率，适合大规模粗筛
- **混合检索**：粗召回用 128 维快筛 → 精排用原始维度做重排序
