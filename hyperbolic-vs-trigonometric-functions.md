...
| 公式 | 含义 |
|------|------|
| $\sinh x = \dfrac{e^x - e^{-x}}{2}$ | 奇分量（指数差） |
| $\cosh x = \dfrac{e^x + e^{-x}}{2}$ | 偶分量（指数和） |
| $\tanh x = \dfrac{e^x - e^{-x}}{e^x + e^{-x}}$ | 比值形式 |
| $e^x = \cosh x + \sinh x$ | 指数拆偶+奇（三角的 $e^{ix}=\cos x+i\sin x$ 的实数类比） |
| $e^{-x} = \cosh x - \sinh x$ | 反向分解 |
| $\cosh^2 x - \sinh^2 x = \frac{(e^x+e^{-x})^2}{4} - \frac{(e^x-e^{-x})^2}{4} = \mathbf{1}$ | 毕达哥拉斯恒等式的指数验证 |

> **对比**：三角函数的对应形式 $e^{ix}=\cos x+i\sin x$ 涉及复数平面旋转；双曲函数的 $e^{x}=\cosh x+\sinh x$ 则是纯实数指数增长。从指数形式看，双曲函数是**指数函数的偶/奇分量分解**——cosh 取和、sinh 取差。
